const ERA_COLORS = {
  'before the college dropout':'#2e1a6e',
  'the college dropout':'#6e3a1a',
  'late registration':'#1a3a6e',
  'graduation':'#5a4a0a',
  '808s':'#0a4a5a',
  'my beautiful dark twisted fantasy':'#2a5a1a',
  'watch the throne':'#5a1a3a',
  'yeezus':'#6e1a1a',
  'the life of pablo':'#4a1a6e',
  'ye':'#0a5a4a',
  'kids see ghosts':'#5a1a5a',
  'jesus is king':'#4a5a0a',
  'donda':'#1a1a2e',
  'vultures':'#5a3a0a',
  'cruel summer':'#6e4a0a',
};
...
function getColor(era) {
  const k = era.toLowerCase();
  for (const [p,c] of Object.entries(ERA_COLORS)) if (k.includes(p)) return c;
  return '#1a1a2e';
}
...
function resetHero() {
  document.getElementById('hero-bg').style.background =
    'radial-gradient(ellipse 60% 80% at 50% 50%,#1a0a2e 0%,#0a0a0a 75%)';
  ...
  keys.forEach(era => {
    ...
    row.onclick = () => {
      ...
      if (!isOpen) {
        ...
        document.getElementById('hero-bg').style.background =
          `radial-gradient(ellipse 60% 80% at 50% 50%, ${getColor(era)} 0%, #0a0a0a 75%)`;
        ...
        if (!panel.dataset.loaded) {
          ...
          songs.forEach(([name, quality, length, url], i) => {
            ...
            el.innerHTML = `
              ...
              <div class="play-btn"><svg viewBox="0 0 10 10"><polygon points="2,1 9,5 2,9"/></svg></div>
            `;
